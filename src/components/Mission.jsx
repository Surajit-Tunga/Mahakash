...
const MissionCard = ({ image, reverse, title, description }) => {
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 items-center ${reverse ? 'sm:flex-row-reverse' : ''}`}>
      {!reverse && <img src={image} alt={title} className="w-full h-auto rounded-lg transform scale-95 mx-auto" />}
      <div className={`space-y-4 xl:pr-36 p-4 border-b-2 border-sky-800 ${reverse ? 'border-l-2' : 'border-r-2'}`}>
        <p className='text-sky-800 uppercase font-semibold'>Our Mission</p>
        <h1 className='uppercase text-4xl md:text-5xl font-bold'>{title}</h1>
        <p className="text-gray-200">{description}</p>
        <button className='bg-white text-black px-4 py-2 rounded-md mt-5 hover:bg-slate-300 transition duration-300'>Learn More</button>
      </div>
      {reverse && <img src={image} alt={title} className="w-full h-auto rounded-lg transform scale-95 mx-auto" />}
    </div>
  );
